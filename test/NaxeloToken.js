...
contract('NaxeloToken', function(accounts) {
    ...
    it('intializes the contract with the correct values', function() {
        return NaxeloToken.deployed().then(function(instance) {
            ...
            assert.equal(name, 'Naxelo Token', 'has the correct name');
            return tokenInstance.symbol();
        }).then(function(symbol) {
            assert.equal(symbol, 'NXLO', 'has the correct symbol');
            return tokenInstance.standard();
        }).then(function(standard) {
            assert.equal(standard, 'Naxelo Token v1.0', 'has the correct standard');
        });
    });
